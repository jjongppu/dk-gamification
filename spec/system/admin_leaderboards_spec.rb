...
describe "Admin leaderboards", type: :system, js: true do
  ...
  it "can create a leaderboard" do
    visit("/admin/plugins/dk-gamification")
    ...
    expect(page).to have_current_path(
      "/admin/plugins/dk-gamification/leaderboards/#{DKGamification::GamificationLeaderboard.last.id}",
    )
    ...
    expect(::DKGamification::GamificationLeaderboard.last).to have_attributes(
      name: "My leaderboard",
  ...
  context "when there is an existing leaderboard" do
    ...
    it "can delete a leaderboard" do
      ...
      expect(::DKGamification::GamificationLeaderboard.exists?(leaderboard.id)).to eq(false)
    end
  end
end
